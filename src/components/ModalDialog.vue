...
const isValidUrl = (url: string): boolean => {
  return url.includes('.com') || url.includes('.in')
}
...
const addLink = () => {
  ...
  linkError.value = !isValid

  if (isValid && !localCard.value.links?.includes(link)) {
...
watch(newLink, (val) => {
  if (linkError.value && isValidUrl(val.trim())) {
    ...
    <div class="bg-white p-5 rounded max-w-md w-full">
      ...
      <div class="mb-4">
        ...
        <div class="flex items-center mb-2">
          ...
          />
          <button @click="addTag" class="bg-green-500 text-white px-3 py-1 rounded">Add</button>
        </div>
        <div class="flex flex-wrap gap-2">
          ...
          >
            {{ tag }}
            <button @click="removeTag(index)" class="ml-1 text-red-500 font-bold">&times;</button>
          </span>
        </div>
      </div>
      <!-- Links Section -->
      ...
      <div class="mb-4">
        ...
        <div class="flex items-center mb-2">
          ...
          <button @click="addLink" class="bg-green-500 text-white px-3 py-1 rounded">Add</button>
        </div>
        <p v-if="linkError" class="text-red-500 text-sm mt-1">Please enter a valid URL.</p>
        ...
        <ul class="space-y-1 text-sm">
          ...
          >
            ...
            <button @click="removeLink(index)" class="ml-2 text-red-500 font-bold">&times;</button>
          </li>
        </ul>
      </div>
      ...
      <div class="mb-4">
        ...
        >
          <input v-model="item.checked" type="checkbox" class="mr-2 accent-green-500" />
          <input
            v-model="item.label"
            type="text"
            placeholder="Label"
          ...
          />
          <button @click="removeCheckbox(index)" class="text-red-500 font-bold">&times;</button>
        </div>
        <button @click="addCheckbox" class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-sm">
          Add Subtask
        </button>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="flex gap-2 flex-wrap">
          ...
          >
            <img :src="img" alt="Uploaded Image" class="object-cover w-full h-full" />
            <button
